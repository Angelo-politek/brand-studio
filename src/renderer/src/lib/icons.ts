 ...
 * so the two sets never collide. SVGs are recolored and rasterized to an
 * HTMLImageElement for Konva (live node + export).
 *
 * SVG markup is lazy-loaded from two generated JSON maps (name -> raw svg)
 * built by scripts/build-icons.mjs (see src/renderer/src/lib/generated/,
 * regenerated by the predev/prebuild/pretest npm hooks). This replaces a
 * previous `import.meta.glob` over the ~5440 individual .svg files, which
 * made Vite/Rollup emit one chunk per icon — bloating the packaged app and
 * slowing startup. The name list (icon-names.json) is tiny and loaded
 * eagerly so the search index below can be built synchronously, same as
 * before; the actual SVG content maps are still fetched lazily, one dynamic
 * import per source instead of one per icon.
 */

const { lucide: lucideNames, simple: simpleNames } = iconNames as {
  lucide: string[]
  simple: string[]
}

let lucideMapPromise: Promise<Record<string, string>> | null = null
let simpleMapPromise: Promise<Record<string, string>> | null = null

function loadLucideMap(): Promise<Record<string, string>> {
  if (!lucideMapPromise) {
    lucideMapPromise = import('./generated/lucide-icons.json').then(
      (m) => (m.default ?? m) as unknown as Record<string, string>
    )
  }
  return lucideMapPromise
}

function loadSimpleMap(): Promise<Record<string, string>> {
  if (!simpleMapPromise) {
    simpleMapPromise = import('./generated/simple-icons.json').then(
      (m) => (m.default ?? m) as unknown as Record<string, string>
    )
  }
...
const lucideEntries: IconEntry[] = lucideNames.map((name) => ({
  id: `lucide:${name}`,
...
const simpleEntries: IconEntry[] = simpleNames.map((slug) => {
...
export function searchIcons(
  ...
  const pool =
    category === 'social'
      ? (SOCIAL_SLUGS.map((s) => simpleEntries.find((e) => e.name === s)).filter(
          Boolean
        ) as IconEntry[])
      : category === 'audio'
        ? ([
...
export async function loadIconSvg(id: string): Promise<string | null> {
  ...
  const map = await (source === 'simple' ? loadSimpleMap() : loadLucideMap())
  const svg = map[name]
  if (!svg) return null
  rawCache.set(id, svg)
  return svg
